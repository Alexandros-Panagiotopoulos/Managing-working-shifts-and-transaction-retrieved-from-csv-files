...
class SolutionTestCase(unittest.TestCase):
    ...
    def test_get_time_for_break_format_with_PM(self):
        break_start, break_end = get_time_in_hours_from_midnight ('4PM-5PM', 0, 1)

        self.assertEqual(break_start, 16)
        self.assertEqual(break_end, 17)

    def test_get_time_with_break_format_with_minutes(self):
        break_start, break_end = get_time_in_hours_from_midnight ('4-4.30PM', 0, 1)
    ...
    def test_get_time_raise_exception_because_of_invalid_break_start_end_separation(self):
        ...
        self.assertEqual("Invalid entry in line "+str(line+1) +" of 'shifts.csv. Please separate the start and end of the break with a '-'", str(error.exception))

    def test_get_time_raise_exception_because_of_invalid_time_stamp_format(self):
        line = 3
        with self.assertRaises(InvalidTimeStampFormating) as error:
            get_time_in_hours_from_midnight ('15h', 1, line)
        self.assertEqual("Invalid entry in line "+str(line+1) +" of 'shifts.csv. An unexpected format at the end of a shift was recieved, please use format like '22:16'", str(error.exception))
